...
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
// MEX implementation of the polyphase network for the synthesis stage  of a 
                  ...
                  int nrhs,const mxArray *prhs[])
{
    ...
    if(nrhs!=8) {
        mexErrMsgTxt("gdft_fb_synthesis_mex.c: 8 inputs are required.");
    } 
    
    if(nlhs!=1) {
        mexErrMsgTxt("gdft_fb_synthesis_mex.c: 1 output required.");
    }
    ...
    fftw_complex *x;
    double *poly_coeff,*poly_tap_in,*poly_branch_in;
    size_t nDim_x,nDim_p1,nDim_p2,nDim_p3; 
    const size_t *size_x,*size_p1,*size_p2,*size_p3; 
       
    // 1st input (input signals [K x Ix x Nx]) 
    x              = (fftw_complex*) mxGetComplexDoubles(prhs[0]);
    ...
    // Check dimensions
    if(nDim_x!=3){
    	mexErrMsgTxt("gdft_fb_synthesis_mex: Incorrect dimensions for x");
    }
    if(size_x[0]!=K){
    	mexErrMsgTxt("gdft_fb_synthesis_mex: Incorrect dimensions for x");
    }
    if(nDim_p1!=2 || nDim_p2!=2 || nDim_p3!=2){
        mexErrMsgTxt("gdft_fb_synthesis_mex: Incorrect dimensions for poly_coeff, poly_tap, or poly_branch");
    }
    ...
    // --------------------------- DEFINE OUTPUT ---------------------------
    // Output (filtered signal [Iy x Nx])
    double *y;  
    int Iy = Ix*R+Ip-1;
    // Set output dimensions
    size_t nDim_y = 2;
    const size_t size_y[2] = {Iy,Nx};
    // Create output array
    plhs[0] = mxCreateNumericArray(nDim_y,
    	                           (const mwSize*) size_y,
    	                           mxDOUBLE_CLASS,mxREAL);
    y       = (double*) mxGetDoubles(plhs[0]);   
