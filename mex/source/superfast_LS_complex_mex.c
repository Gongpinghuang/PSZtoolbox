...
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
// MEX implementation of the superfast solver for complex-valued LS problems
// proposed in:
//
                  ...
                  int nrhs,const mxArray *prhs[])
{
    ...
    if(nrhs!=8) {
        mexErrMsgTxt("superfast_LS_complex_mex.c: 8 inputs are required.");
    } 
    
    if(nlhs!=1) {
        mexErrMsgTxt("superfast_LS_complex_mex.c: 1 output required.");
    }
    ...
    if(nDim_h!=3 && nDim_h!=2){
        mexErrMsgTxt("superfast_LS_complex_mex.c: Incorrect size for h or d"); 
    }
    if(size_h[1]!=L || size_h[2]!=M || size_d[1]!=M){
        mexErrMsgTxt("superfast_LS_complex_mex.c: Incorrect size for h or d"); 
    }
    if(beta<=0.0){
        mexErrMsgTxt("superfast_LS_complex_mex.c: Incorrect value for beta"); 
    }
    if(P<0){
        mexErrMsgTxt("superfast_LS_complex_mex.c: Incorrect approx. order"); 
    }
    ...
    g       = (fftw_complex*) mxGetComplexDoubles(plhs[0]);   

    // ------------------------------ COMPUTE ------------------------------ 
    // Solve LS problem
    superfast_LS_complex(h,d,L,M,Ih,Ig,Id,Nfft,P,beta,g);
}
