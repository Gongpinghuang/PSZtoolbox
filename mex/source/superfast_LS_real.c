...
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
// C implementation of the superfast solver for real-valued LS problems
// proposed in:
//
...
// -------------------------------------------------------------------------
void superfast_LS_real(double *h,
                       double *d,
                       const int L,
                       const int M,
                       const int Ih,
    ...
    // Check
    if(Nfft<Id || Nfft%2!=0){
    	mexErrMsgTxt("superfast_LS_real.c: Incorrect FFT size");
    }
    // FFT scaling
    ...
    for(int i=0;i<M*L;i++){
      ...
      matC_copy_step(X,hf+M*l+m,Kfft,1,M*L);
    }
    ...
    double *g_fd  = calloc(Nfft*L,sizeof(double)); 
    // For each loudspeaker...
    for(int l=0;l<L;l++){
      // Compute filters time domain 
      fftw_execute_dft_c2r(fftPlan_inv,gf+Kfft*l,g_fd+Nfft*l);
      // Correct fft scaling
      matR_scale(g_fd+l*Nfft,Nfft,fft_sc);
    }
    // Free memory
    fftw_free(Rf);
    fftw_free(Hf_ps);
    ...
    double *rt           = calloc(Nfft*L,sizeof(double)); 
    fftw_complex *rf_aux = fftw_malloc(Kfft*L*sizeof(fftw_complex));
    fftw_complex *rf     = fftw_malloc(Kfft*L*sizeof(fftw_complex));

    // Obtain initial time-domain residuals for this iteration
    // For each loudspeaker...
    for(int l=0;l<L;l++){
      memcpy(rt+l*Nfft,g_fd+l*Nfft,Nfft*sizeof(double));
    }
    ...
    for(int p=0;p<P;p++){
        ...
        for(int l=0;l<L;l++){
            // Set to 0
            memset(rt+Nfft*l,0.0,Ig*sizeof(double));
            // Compute FFT
            fftw_execute_dft_r2c(fftPlan,rt+Nfft*l,rf_aux+Kfft*l);
        }
        ...
        for(int l=0;l<L;l++){
            ...
            fftw_execute_dft_c2r(fftPlan_inv,rf+Kfft*l,rt+Nfft*l);
            // Correct fft scaling
      	  	matR_scale(rt+l*Nfft,Nfft,fft_sc);
    ...
    for(int l=0;l<L;l++){
      ...
      memcpy(g+l*Ig,g_fd+l*Nfft,Ig*sizeof(double));
    }
